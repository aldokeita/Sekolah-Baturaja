    ...
    <div style={{ position: 'sticky', top: 0, zIndex: 45, padding: '12px 16px 0' }}>
      <div
        style={{
          ...
          backdropFilter: 'blur(26px) saturate(185%)', WebkitBackdropFilter: 'blur(26px) saturate(185%)',
          border: '1px solid rgba(255,255,255,.7)',
          boxShadow: '0 18px 40px -20px rgba(55,65,120,.5),inset 0 1px 0 rgba(255,255,255,.95)',
        }}
      >
        <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 11, minWidth: 0, color: 'inherit' }}>
          <div style={{ flex: 'none', width: 36, height: 36, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, color: '#fff', background: 'linear-gradient(140deg,var(--sekolah-aksen),var(--sekolah-aksen-tengah-2) 45%,var(--sekolah-aksen-ujung))', boxShadow: '0 10px 20px -8px rgba(110,120,220,.75),inset 0 1px 0 rgba(255,255,255,.8)' }}>
            {sekolah.logoAbbr}
          </div>
          <div style={{ minWidth: 0, lineHeight: 1.2 }}>
            <div style={{ fontSize: 13.5, fontWeight: 800, letterSpacing: '-.01em', color: '#1b1c28', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {sekolah.shortName}
            </div>
            <div style={{ fontSize: 11, fontWeight: 500, color: '#6c718f' }}>
        ...
        <button type="button" onClick={toggleTheme} aria-label="Ganti tema terang atau gelap" style={{ ...tombol, width: 38, padding: 0, justifyContent: 'center' }}>
        ...
        <Link to="/" aria-label="Lihat situs sekolah" style={tombol}>
          <ExternalLink className="h-4 w-4" aria-hidden="true" />
          <span className="dash-topbar__label">Lihat situs</span>
        </Link>

        <button type="button" onClick={handleLogout} aria-label="Keluar dari akun" style={{ ...tombol, color: '#b04a5a' }}>
