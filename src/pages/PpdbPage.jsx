...
const chipStyle = (on) => 'position:relative;overflow:hidden;padding:12px 20px;border-radius:14px;cursor:pointer;font-family:inherit;font-size:13.5px;font-weight:700;transition:all .2s ease;border:1px solid '
  ...
  + (on ? 'linear-gradient(135deg,#6470ff,#a06cf0)' : 'rgba(255,255,255,.62)') + ';box-shadow:'
...
const PpdbPage = () => {
  ...
  const kirim = async () => {
    const ringkas = [
      ...
      `Asal TK/RA: ${v('sekolah')} (NPSN ${v('npsn')})`, `Usia per 1 Juli 2026: ${v('nilai')}`,
    ...
    try {
      await submitPublicFeedback({
        ...
        email: d.email?.trim() || '',
        no_hp: d.hp?.trim() || '',
        pesan: `[Pendaftaran PPDB 2026/2027]\n${ringkas}`,
      });
  ...
  const vals = {
    ...
    steps: STEP_DEFS.map(([n, label, hint]) => {
      ...
      return {
        ...
        dot: `flex:none;width:30px;height:30px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;color:${active || passed ? '#fff' : '#6b7093'};background:${active ? 'linear-gradient(135deg,#6470ff,#a06cf0)' : passed ? 'linear-gradient(135deg,#7fd8c0,#6fc9e0)' : 'rgba(255,255,255,.72)'};box-shadow:inset 0 1px 0 rgba(255,255,255,.7)`,
    ...
    jalur: JALUR_DEFS.map(([k, label, desc]) => {
      ...
      return {
        ...
        mark: `flex:none;margin-top:2px;width:18px;height:18px;border-radius:50%;border:2px solid ${on ? '#6470ff' : 'rgba(120,127,160,.4)'};background:${on ? 'radial-gradient(circle,#6470ff 0 42%,transparent 44%)' : 'transparent'}`,
      };
    }),
    ...
    review: [
      ...
      { k: 'Usia per 1 Juli 2026', v: v('nilai') },
    ...
    progressStyle: `height:100%;width:${pct}%;border-radius:99px;background:linear-gradient(90deg,#6470ff,#a06cf0 55%,#f090c0);transition:width .5s cubic-bezier(.3,.8,.3,1);box-shadow:0 0 12px rgba(120,132,255,.6)`,
    ...
    nextStyle: `position:relative;overflow:hidden;display:inline-flex;align-items:center;gap:9px;padding:14px 24px;border-radius:15px;border:0;font-family:inherit;font-size:14px;font-weight:700;color:#fff;background:linear-gradient(135deg,#5b6cff,#8a6cf0 55%,#e58fc4);box-shadow:0 20px 40px -16px rgba(90,100,235,.95),inset 0 1px 0 rgba(255,255,255,.6);transition:opacity .2s ease;cursor:${step === 4 && !setuju ? 'not-allowed' : 'pointer'};opacity:${step === 4 && !setuju ? '.42' : '1'}`,
    ...
    setujuBox: `flex:none;width:22px;height:22px;border-radius:7px;display:flex;align-items:center;justify-content:center;transition:all .2s ease;border:1px solid ${setuju ? 'transparent' : 'rgba(120,127,160,.4)'};background:${setuju ? 'linear-gradient(135deg,#6470ff,#a06cf0)' : 'rgba(255,255,255,.7)'}`,
  ...
  return (
    <div className="sdnb sdnb-ppdb">
      <Helmet>
        <title>Formulir PPDB — Sekolah Dasar Negeri Baturaja</title>
        <meta name="description" content="Formulir pendaftaran peserta didik baru 2026/2027 Sekolah Dasar Negeri Baturaja." />
      </Helmet>
      {PpdbBody(vals)}
    </div>
  );
